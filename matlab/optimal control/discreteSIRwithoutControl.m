function [y,k]=discreteSIRwithoutControl(LAMBDA,beta,mu,d,r,S0,I0,R0,A)
...
while(test<0)
    ...
    for i=1:n
       lambda1(n-i+1)=-(mu-1)*lambda1(n-i+2)+beta*(I(n-i+1)/N(n-i+1))*(lambda2(n-i+2)-lambda1(n-i+2));
       lambda2(n-i+1)=A+beta*(S(n-i+1)/N(n-i+1))*(lambda2(n-i+2)-lambda1(n-i+1))-(mu+d+r-1)*lambda2(n-i+2)+r*lambda3(n-i+2);
