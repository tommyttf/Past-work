function [y,k]=discreteSIRwithControl(LAMBDA,beta,mu,d,r,S0,I0,R0,A,B,a,b)
...
while(test<0)
    ...
    for i=1:T
       lambda1(T-i+1)=-(mu-1)*lambda1(T-i+2)+beta*(I(T-i+1)/N(T-i+1))*(lambda2(T-i+2)-lambda1(T-i+2))+u(T-i+1)*(lambda3(T-i+2)-lambda1(T-i+2));
       lambda2(T-i+1)=A+beta*(S(T-i+1)/N(T-i+1))*(lambda2(T-i+2)-lambda1(T-i+1))-(mu+d+r-1)*lambda2(T-i+2)+r*lambda3(T-i+2);
       lambda3(T-i+1)=-(mu-1)*lambda3(T-i+2);
    end
    

    for i=1:T+1
        M(i)=((lambda1(i)-lambda3(i))/B)*S(i);
        u(i)=min(b,max(a,M(i)));
    end
