function [y,k]=SEIRwithControl(LAMBDA1,LAMBDA2,beta,d,alpha,epsilon,gamma,S0,E0,I0,R0,A1,A2,n)
...
while(test<0)
    ...
    for i =0:n-1
       lambda1(n-i)=(lambda1(n-i+1)+h*(1-u(n-i))*lambda2(n-i+1)*beta*(I(n-i)/N(n-i)))/(1+h*(d+(1-u(n-i))*beta*(I(n-i)/N(n-i))));
       lambda2(n-i)=(lambda2(n-i+1)+h*(1+alpha*lambda3(n-i+1)))/(1+h*(alpha+d));
       lambda3(n-i)=(lambda3(n-i+1)+h*(1+(lambda1(n-i)-lambda2(n-i))*(1-u(n-i))*beta*(S(n-i)/N(n-i))))/(1+h*(epsilon+gamma+d+v(n-i)));       
       lambda4(n-i)=lambda4(n-i)/(1+h*d);
    end
    
    for i=1:n+1
        M(i)=((lambda2(i)-lambda1(i))/A1)*beta*(I(i)*S(i)/N(i));
        u(i)=min(1,max(0,M(i)));
        T(i)=((lambda3(i)-lambda4(i))/A2)*I(i);
        v(i)=min(1,max(0,T(i)));
    end
